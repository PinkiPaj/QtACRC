...
#                 # ширина,  высота
#                 # width,   height
#                 img = img.resize((int(per1), int(per2)), Image.LANCZOS)
#                 if x[-4:] == ".PNG":
#                     img.save(f'output\\{(tree[i][0])[6:]}\\{x}')
#                 else:
#                     img.save(f'output\\{(tree[i][0])[6:]}\\{x[:x.find('.')]}.PNG')
#     else:
#         print('There are no files in the input folder')
#     print(input('Complet, press Enter to exit'))